...
bool FileConfigLoader::load()
{
    ...
    const auto& simulation = jc["Drone"]["simulation"];
    config_.simTimeStep = simulation.value("timeStep", 0.1f);
    config_.physicsTimeStep = simulation.value("physicsTimeStep", 0.01f);
    config_.timeScale = simulation.value("timeScale", 10.0f);
    config_.hitRadius = simulation.value("hitRadius", 2.0f);
