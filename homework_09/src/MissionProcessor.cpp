...
MissionProcessor::MissionProcessor(std::unique_ptr<IConfigLoader> loader, std::unique_ptr<ITargetProvider> provider, std::unique_ptr<IBallisticSolver> solver)
    : loader_(std::move(loader)), provider_(std::move(provider)), solver_(std::move(solver))
{
}
...
void MissionProcessor::step()
{
    ...
    simSteps_[stepIndex_].state = state_ ? state_->getStateId() : 0;
    ...
    DroneStateContext ctx;
    ctx.currentPos = currentPos_;
    ctx.direction = direction_;
    ctx.speed = speed_;
    ctx.currentTime = currentTime_;
    ctx.targetPos = bestFirePos;
    ctx.desiredDirection = desiredDirectionForBest;
    ctx.dt = cfg_.simTimeStep;
    ...
    auto nextState = state_->update(ctx);
    if (nextState)
    {
        state_ = std::move(nextState);
    }

    currentPos_ = ctx.currentPos;
    direction_ = ctx.direction;
    speed_ = ctx.speed;
    currentTime_ = ctx.currentTime + cfg_.simTimeStep;

    Coord aimPointAfterMove = calcAimPoint(currentPos_, direction_, ballistic_.hDistance);

    if (ctx.targetReached || (state_ && state_->getStateId() == 4 &&
