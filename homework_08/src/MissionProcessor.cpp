...
void MissionProcessor::step()
{
    ...
    std::vector<int> targetId(provider_->getTargetCount());

    for (const auto& targetId: targetId)
    {
        ...
        //create range based for iter
        std::vector<int> iter(NUM_TIME_APPROXIMATION_STEPS);

        for (const auto& iter : iter)
        {
