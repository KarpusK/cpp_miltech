...
struct BallisticTable {
    ...
    Result lookup(float Z0, float V0, float m,
                  float d,  float l) const;

    std::string tablepath = "data/ballistic_table.txt";
...
// Допоміжні функції для інтерполяції
inline BallisticTable::Result lerp(const BallisticTable::Result& a, const BallisticTable::Result& b, float t) {
    return {
        a.t     + (b.t     - a.t)     * t,
...
// Реалізація lookup методу
inline BallisticTable::Result BallisticTable::lookup(
...
// TableSolver клас, який реалізує IBallisticSolver
class TableSolver : public IBallisticSolver
{
private:
    mutable BallisticTable table;
