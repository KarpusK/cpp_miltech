...
class GimbalDriver {
public:
  explicit GimbalDriver(float max_step_per_tick = 5.0F, float neutral_angle = 90.0F)
    : max_step_per_tick_(max_step_per_tick)
    , neutral_angle_(neutral_angle)
    , target_angle_(neutral_angle)
  {}

  void update_from_error(float error_y)
  {
    const auto clamped_error = std::clamp(error_y, -max_step_per_tick_, max_step_per_tick_);
